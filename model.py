...
class LabelEmbedder(nn.Module):
    ...
    def __init__(self, num_classes, hidden_size, dropout_prob):
        super().__init__()
        use_cfg_embedding = dropout_prob > 0 or True
...
class ScaleAwareSiT(SiT):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.s_embedder = TimestepEmbedder(kwargs['hidden_size'])
        nn.init.normal_(self.s_embedder.mlp[2].weight, std=0.0002)
        nn.init.constant_(self.s_embedder.mlp[2].bias, 0)

    def forward(self, x, t, y, s=0):
        """
        Forward pass of SiT.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        s: (N,) tensor of classifier-free guidance scale
        y: (N,) tensor of class labels
        """
        if not isinstance(s, torch.Tensor):
            s = torch.ones_like(t) * s
        x = self.x_embedder(x) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        t = self.t_embedder(t)  # (N, D)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        return self.forward(x, t, y, s=cfg_scale)


def ScaleAware_SiT_XL_2(**kwargs):
    return ScaleAwareSiT(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)


def ScaleAware_SiT_L_2(**kwargs):
    return ScaleAwareSiT(depth=24, hidden_size=1024, patch_size=2, num_heads=16, **kwargs)


def ScaleAware_SiT_B_2(**kwargs):
    return ScaleAwareSiT(depth=12, hidden_size=768, patch_size=2, num_heads=12, **kwargs)
...
SiT_models = {
    ...
    'ScaleAware-SiT-XL/2': ScaleAware_SiT_XL_2,
